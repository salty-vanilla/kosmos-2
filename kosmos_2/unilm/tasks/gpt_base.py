...
class GPTTask(FairseqTask):
    ...
    def setup_task(cls, cfg, **kwargs):
        ...
        if getattr(cfg, 'quantized_size', None) is not None:
            # location task, add patch index token as special symbols
            for special_symbol in add_location_symbols(cfg.quantized_size,
                                                       getattr(cfg, 'locate_special_token', 0)):
        ...
        logger.info("dictionary enriched with some special tokens: {} types".format(len(dictionary)))
